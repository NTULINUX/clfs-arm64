...
if [ ! -f $SYSIMG ]; then
  ...
  test -f $SYSTEM/usr/bin/login || build_shadow || die "build_shadow"
  test -f $SYSTEM/usr/bin/ps || build_procps || die "build_procps"
  test -f $SYSTEM/sbin/udevd || build_eudev || die "build_eudev"
  test -f $SYSTEM/sbin/init || build_sysvinit || die "build_sysvinit"
  ...
  test -f $SYSTEM/etc/rc.d/init.d/rc || build_bootscript || die "build_bootscript"
...
#  test -f $SYSTEM/usr/bin/vim || build_vim || die "build_vim"
